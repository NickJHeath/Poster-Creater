...
function getList(files, folder, type) {
  ...
  let pptxList = files.filter(
    (file) =>
      file.match(/\.[0-9a-z]+$/i) && file.match(/\.[0-9a-z]+$/i)[0] === ".pptx"
  );
  let list = pptxList.map((file) => {
    let pdf = file.replace(/\.pptx/g, ".pdf");
    let jpg = file.replace(/\.pptx/g, ".jpg");
    if (type === ".pdf")
      return { in: formattedFolder + file, out: formattedFolder + pdf };
    else
      return {
        in: formattedFolder + pdf,
        out: formattedFolder + "posters\\" + jpg,
      };
  });
  return list;
}
...
async function findLO() {
  let options = {
    ...
    message:
      "Please select soffice.bin within LibreOffice install directory (usually within Program Files directory)",
  };
  dialog.showMessageBoxSync(null, {
    title: "Folder prompt",
    message:
      "Please select the soffice.bin file from within your LibreOffice install directory (file is usually found within LibreOffice\\Program directory)",
    buttons: ["OK"],
  });
  let selection = await dialog.showOpenDialog(options);
  if (selection.canceled === true) {
...
async function getFiles(selection) {
  let folder = selection.filePaths[0];
  let files = await listFilesAsync(folder);
  return files;
}
...
function buildList(files, folder) {
  let fileList = [".pdf", ".jpg"];
  let list = [];
  for (let type of fileList) {
...
if ($("#getFolder").length) {
  $("#getFolder").click(async () => {
    let selection = await chooseFiles();
    if (selection.canceled === true) {
      return;
    }
    ...
    if (!fs.existsSync("C:\\Program Files\\LibreOffice\\program\\soffice.bin"))
      LOloc = await findLO();
    let files = await getFiles(selection);
    let dir = selection.filePaths[0];
    let finalFiles = buildList(files, dir);
